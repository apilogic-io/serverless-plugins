...
export class PathManager {
  ...
  getAmplifyAdminDirPath = (): string =>
    this.constructPath(this.getHomeDotAmplifyDirPath(), [PathConstants.AmplifyAdminDirName]);
  ...
  getCurrentResourceParametersJsonPath = (
  ...
  getCurrentCfnTemplatePath = (projectPath: string | undefined, categoryName: string, resourceName: string): string =>
    path.join(
      ...
      categoryName,
      resourceName,
      PathConstants.CfnFileName(resourceName)
    );

  getAmplifyRcFilePath = (projectPath?: string): string =>
    this.constructPath(projectPath, [PathConstants.AmplifyRcFileName]);

  getGitIgnoreFilePath = (projectPath?: string): string =>
    this.constructPath(projectPath, [PathConstants.GitIgnoreFileName]);
  ...
  getProjectConfigFilePath = (projectPath?: string): string =>
    this.constructPath(projectPath, [
      ...
      PathConstants.DotConfigDirName,
      PathConstants.ProjectConfigFileName,
    ]);
  ...
  getLocalEnvFilePath = (projectPath?: string): string =>
    this.constructPath(projectPath, [
      PathConstants.AmplifyDirName,
      PathConstants.DotConfigDirName,
      PathConstants.LocalEnvFileName,
    ]);
  ...
  getLocalAWSInfoFilePath = (projectPath?: string): string =>
    this.constructPath(projectPath, [
      PathConstants.AmplifyDirName,
      PathConstants.DotConfigDirName,
      PathConstants.LocalAWSInfoFileName,
    ]);
  ...
  getAWSCredentialsFilePath = (): string =>
    ...
    path.normalize(path.join(this.getDotAWSDirPath(), PathConstants.AWSCredentials));
  ...
  getCLIJSONFilePath = (projectPath: string, env?: string): string => {
    const fileName =
      env === undefined ? PathConstants.CLIJSONFileName : PathConstants.CLIJsonWithEnvironmentFileName(env);

    return this.constructPath(projectPath, [PathConstants.AmplifyDirName, fileName]);
  };

  getDotAWSAmplifyDirPath = (): string =>
    path.normalize(path.join(homedir(), PathConstants.DotAWSDirName, PathConstants.AmplifyDirName));

  getDeploymentSecrets = (): string =>
    path.normalize(path.join(this.getDotAWSAmplifyDirPath(), PathConstants.DeploymentSecretsFileName));
    ...
    if (fs.existsSync(projectPath)) {
      ...
      if (fs.existsSync(amplifyDirPath) && fs.existsSync(dotConfigDirPath)) {
        ...
        if (fs.existsSync(localEnvFilePath)) {
          return projectPath === this.getLocalEnvInfo(projectPath).projectPath;
        }
      }
    }
  ...
  getLocalEnvInfo = (projectPath?: string, options?: any): any => {
  ...
  private getData = <T>(filePath: string, options?: any): T | undefined => {
