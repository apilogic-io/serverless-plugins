...
export default class StepFunctionsOfflinePlugin implements ServerlessPlugin {
  ...
  private readonly detailedLog: Options['detailedLog'];
  eventFile: Options['eventFile'] | Options['ef'];
  event: Options['event'] | Options['e'];
  ...
  _checkVersion(): void {
    ...
    if (!version.startsWith('3.')) {
      throw new Error(`Serverless Step Functions Offline requires Serverless v3.x.x but found ${version}`);
    }
  }
  ...
  getStateMachine(stateMachineName: string): StateMachineBase {
    ...
    throw new Error(`stateMachine "${stateMachineName}" doesn't exist in this Service`);
  }
  ...
  async _loadStates(states: StateMachine['States'] | string): Promise<StateMachine['States']> {
    if (isString(states)) {
      ...
      if (!match) {
        throw new Error(`couldn't understand string of States: ${states}`);
      }
      const fileName = match[1];
      if (!this.serverlessFileExists(fileName)) {
        throw new Error(`Unable to find ${fileName} in serverless path`);
      }
  ...
  addContextObject(states: StateMachine['States'], name: string, event: Event): void {
    ...
    this.contexts[name] = this.createContextObject(states, name, event);
  }
